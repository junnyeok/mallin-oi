...
-- 계정별 게시글 읽음 기록 테이블
-- =========================================================
create table if not exists public.user_post_reads (
  reader_user_id uuid not null references auth.users(id) on delete cascade,
  post_id bigint not null references public.posts(id) on delete cascade,
  read_at timestamptz not null default now(),
  created_at timestamptz not null default now(),
  primary key (reader_user_id, post_id)
);

create index if not exists user_post_reads_reader_idx
  on public.user_post_reads(reader_user_id, read_at desc);

create index if not exists user_post_reads_post_idx
  on public.user_post_reads(post_id);

alter table public.user_post_reads enable row level security;

drop policy if exists "게시글 읽음 기록은 본인만 조회 가능" on public.user_post_reads;
create policy "게시글 읽음 기록은 본인만 조회 가능"
on public.user_post_reads
for select
to authenticated
using (auth.uid() = reader_user_id);

drop policy if exists "게시글 읽음 기록은 본인만 등록 가능" on public.user_post_reads;
create policy "게시글 읽음 기록은 본인만 등록 가능"
on public.user_post_reads
for insert
to authenticated
with check (auth.uid() = reader_user_id);

drop policy if exists "게시글 읽음 기록은 본인만 수정 가능" on public.user_post_reads;
create policy "게시글 읽음 기록은 본인만 수정 가능"
on public.user_post_reads
for update
to authenticated
using (auth.uid() = reader_user_id)
...
drop policy if exists "게시글 읽음 기록은 본인만 삭제 가능" on public.user_post_reads;
create policy "게시글 읽음 기록은 본인만 삭제 가능"
on public.user_post_reads
for delete
to authenticated
using (auth.uid() = reader_user_id);

grant select, insert, update, delete on public.user_post_reads to authenticated;

-- =========================================================
-- 안 읽은 게시글 수 계산 함수
-- =========================================================
drop function if exists public.get_unread_post_count();

create or replace function public.get_unread_post_count()
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  return (
    ...
    from public.posts_public_list p
    where not exists (
      select 1
      from public.user_post_reads r
      where r.reader_user_id = auth.uid()
        and r.post_id = p.id
    )
  );
end;
...
grant execute on function public.get_unread_post_count() to authenticated;

-- =========================================================
-- 특정 게시글 읽음 처리 함수
-- =========================================================
drop function if exists public.mark_post_as_read(bigint);

create or replace function public.mark_post_as_read(p_post_id bigint)
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into public.user_post_reads (
    reader_user_id,
    post_id,
    read_at
  )
  ...
  on conflict (reader_user_id, post_id)
  do update set
...
grant execute on function public.mark_post_as_read(bigint) to authenticated;

-- =========================================================
-- 1) 계정별 게시글 읽음 기준선 테이블
-- =========================================================
create table if not exists public.user_post_read_state (
...
create index if not exists user_post_read_state_baseline_idx
  on public.user_post_read_state (baseline_created_at desc);

alter table public.user_post_read_state enable row level security;

drop policy if exists "게시글 읽음 기준선은 본인만 조회 가능" on public.user_post_read_state;
create policy "게시글 읽음 기준선은 본인만 조회 가능"
on public.user_post_read_state
for select
to authenticated
using (auth.uid() = user_id);

drop policy if exists "게시글 읽음 기준선은 본인만 등록 가능" on public.user_post_read_state;
create policy "게시글 읽음 기준선은 본인만 등록 가능"
on public.user_post_read_state
for insert
to authenticated
with check (auth.uid() = user_id);

drop policy if exists "게시글 읽음 기준선은 본인만 수정 가능" on public.user_post_read_state;
create policy "게시글 읽음 기준선은 본인만 수정 가능"
on public.user_post_read_state
for update
to authenticated
using (auth.uid() = user_id)
...
grant select, insert, update on public.user_post_read_state to authenticated;

-- =========================================================
-- 2) updated_at 자동 갱신 트리거
-- =========================================================
create or replace function public.handle_user_post_read_state_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists trg_user_post_read_state_updated_at on public.user_post_read_state;
create trigger trg_user_post_read_state_updated_at
before update on public.user_post_read_state
for each row
execute function public.handle_user_post_read_state_updated_at();

-- =========================================================
-- 3) 계정 baseline 보장 함수
-- - 최초 1회만 현재 최신 게시글 시각을 baseline으로 저장
-- - 변수 없이 insert ... select 방식으로 처리
-- =========================================================
drop function if exists public.ensure_user_post_read_state();

create or replace function public.ensure_user_post_read_state()
returns boolean
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into public.user_post_read_state (
    user_id,
    baseline_created_at
  )
  select
    auth.uid(),
    coalesce(max(p.created_at), now())
  from public.posts p
...
grant execute on function public.ensure_user_post_read_state() to authenticated;

-- =========================================================
-- 4) 안 읽은 게시글 수 함수
-- - baseline 이후 생성된 글만 unread 대상
-- - 이미 읽은 글은 제외
-- =========================================================
drop function if exists public.get_unread_post_count();
...
begin
  ...
  perform public.ensure_user_post_read_state();

  return (
    select count(*)::integer
    from public.posts_public_list p
    join public.user_post_read_state s
      on s.user_id = auth.uid()
    where p.created_at > s.baseline_created_at
      and not exists (
        select 1
        from public.user_post_reads r
        where r.reader_user_id = auth.uid()
