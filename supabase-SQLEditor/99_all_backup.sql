...
create or replace function public.save_study_calendar_todo(
  p_todo_id uuid, p_todo_text text, p_memo text,
  p_todo_date date, p_category_id uuid
)
...
begin
  ...
  if not exists (select 1 from public.study_calendar_todos where id = p_todo_id and user_id = v_uid) then
    raise exception '본인 소유의 일정을 찾을 수 없습니다.';
  end if;
  if not exists (select 1 from public.study_calendar_categories where id = p_category_id and user_id = v_uid) then
    raise exception '사용할 수 없는 카테고리입니다.';
  end if;
  perform public.update_study_calendar_todo_category_with_shared_personal(p_todo_id, p_category_id);
  perform public.update_study_shared_personal_todo(p_todo_id, trim(p_todo_text), coalesce(p_memo, ''), p_todo_date);
end;
$$;

create or replace function public.save_work_calendar_todo(
  p_todo_id uuid, p_work_text text, p_memo text, p_work_date date,
  p_category_id uuid, p_overwrite boolean default false
)
returns void language plpgsql security definer set search_path = public
as $$
declare
...
begin
  ...
  select t.* into v_selected from public.work_calendar_todos t
  where t.id = p_todo_id and t.user_id = v_uid for update;
  if not found then raise exception '본인 소유의 일정을 찾을 수 없습니다.'; end if;
  if not exists (select 1 from public.work_calendar_categories where id = p_category_id and user_id = v_uid) then
    raise exception '사용할 수 없는 카테고리입니다.';
  end if;
  v_root_id := coalesce(v_selected.shared_origin_todo_id, v_selected.id);
  select coalesce(array_agg(t.id), array[]::uuid[]) into v_family_ids
  from public.work_calendar_todos t where t.id = v_root_id or t.shared_origin_todo_id = v_root_id;
  ...
  join public.work_calendar_todos t on t.user_id = moving.user_id
    and t.work_date = p_work_date and not (t.id = any(v_family_ids))
  ...
  if cardinality(v_conflict_roots) > 0 then
    if exists (
      select 1 from public.work_calendar_todos conflict_root
    ...
    delete from public.calendar_group_shared_events e
    where e.calendar_type = 'work' and (
      e.source_event_id in (select conflict_root_id::text from unnest(v_conflict_roots) as roots(conflict_root_id))
      or e.payload->>'shared_origin_todo_id' in (select conflict_root_id::text from unnest(v_conflict_roots) as roots(conflict_root_id))
    );
    delete from public.work_calendar_todos t
    where t.id = any(v_conflict_roots) or t.shared_origin_todo_id = any(v_conflict_roots);
  end if;
  perform public.update_work_calendar_todo_category_with_shared_personal(p_todo_id, p_category_id);
  perform public.update_work_shared_personal_todo(
    p_todo_id, coalesce(nullif(trim(p_work_text), ''), v_selected.work_text),
    coalesce(p_memo, ''), p_work_date
  );
end;
$$;
