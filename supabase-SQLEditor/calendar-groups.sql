...
-- =========================================
-- 2026-06-23 우리일정 백업 중복/개인행 잔존 방지
...
begin
  ...
  if p_calendar_type = 'event' then
    ...
    where stale.group_id = p_group_id
      ...
      and not exists (
        select 1
        from public.event_calendar_todos source
        where source.id::text = stale.source_event_id
      )
      and exists (
        select 1
        from public.event_calendar_todos root
  ...
  if p_calendar_type = 'study' then
    ...
    select
      ...
      coalesce(c.slug, t.todo_type), t.todo_text, coalesce(t.memo, ''),
      c.color,
      jsonb_build_object(
        'isDone', t.is_done,
        'categoryName', c.name,
    ...
    select
      ...
      coalesce(c.slug, t.event_type), t.event_text, coalesce(t.memo, ''),
      c.color,
      jsonb_build_object(
        'isDone', t.is_done,
...
begin
  ...
  select
    ...
    coalesce(
      st.shared_origin_todo_id, wt.shared_origin_todo_id, et.shared_origin_todo_id,
      nullif(e.payload->>'shared_origin_todo_id', '')::uuid
    ),
    coalesce(
      st.shared_origin_user_id, wt.shared_origin_user_id, et.shared_origin_user_id,
