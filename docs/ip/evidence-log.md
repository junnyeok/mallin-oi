...
이 로그의 캐릭터·도형 관련 기록은 현재 신청 대상이 아니라 미래 검토자료로 보존한다. 현재 진행 범위는 `말린오이` 문자상표 국내 출원 준비이며, 아래 해시·Git 기록은 삭제하거나 소급 변경하지 않는다.
...
동일 해시는 현재 바이트가 완전히 같다는 의미다. 향후 검토를 재개할 경우 원본 후보와 배포 복제본 관계를 먼저 확인한다.
...
Android·iOS의 해상도별 생성 아이콘과 스플래시는 기존 52행 CSV에 모두 나열하지 않았다. 모바일 패키지의 배포 산출물이며, 향후 검토를 재개할 때는 위 원본용 이미지와 생성 관계를 우선 확인한다.
