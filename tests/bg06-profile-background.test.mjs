...
const itemId = 'BG-06';
const desktop = '/images/profile-background/DAEGU.png?v=test';
const mobile = '/images/profile-background/DAEGU-mobile.png?v=test';
const editorPath = 'supabase-SQLEditor/20260913-dgb-park-profile-background.sql';
const migrationPath = 'supabase/migrations/20260913020000_bg06_dgb_park_profile_background.sql';

test('DGB PARK가 신규 아지트 다음이며 538피클·지정 문구·두 미리보기가 정확하다', () => {
  const item = catalog.getStoreItemById(itemId);
  assert.equal(catalog.getFeaturedStoreItems(2)[1].id, itemId);
  assert.equal(item.name, 'DGB PARK');
  ...
  assert.equal(item.price, 538);
  assert.equal(item.state, '판매 중');
  assert.equal(item.isPurchasable, true);
  assert.equal(item.description, '대구FC의 경기장이야.');
  ...
  assert.deepEqual(item.previewImages, [
    {code: 'BG-06-pc', label: 'PC 버전 미리보기', imagePath: desktop, displayOrder: 1},
    {code: 'BG-06-mobile', label: '모바일 버전 미리보기', imagePath: mobile, displayOrder: 2},
  ]);
  assert.equal(item.releaseDate, '2026-09-13');
  assert.equal(catalog.STORE_ITEMS.toSorted((a,b) => String(b.releaseDate).localeCompare(String(a.releaseDate)))[1].id, itemId);
  assert.equal(catalog.getStoreItemDetailHref(itemId), './store-item.html?id=BG-06');
});

test('프로필배경 카탈로그에서 PC·모바일 파일을 구분하며 ID와 순서가 중복되지 않는다', () => {
  assert.deepEqual(catalog.getProfileBackgroundByItemId(itemId), {
    itemId, name:'DGB PARK', pcImagePath:desktop, mobileImagePath:mobile,
    thumbImagePath:desktop, displayOrder:6,
  });
  for (const values of [
...
test('PC·모바일 PNG 원본이 디코딩 가능하며 앱 복사본·파일 목록과 일치한다', async () => {
  ...
  for (const [name,width,height] of [['DAEGU.png',1499,1049],['DAEGU-mobile.png',958,1642]]) {
  ...
  for(const path of [editorPath,migrationPath,'tests/bg06-profile-background.test.mjs']) assert.equal(files.filter(line=>line===`./${path}`).length,1);
...
test('구매 SQL 네 관리 지점에 가격·지급·우회 금지·원자적 원장이 일치한다', async () => {
  ...
  assert.match(sql,/p_item_id = 'BG-06' then\s+v_price := 538;\s+v_name := 'DGB PARK';\s+v_category := 'profile';/);
  assert.match(sql,/p_item_id = 'BG-06' then\s+v_can_bypass_store_balance := false;/);
  assert.match(sql,/v_is_auto_topup_admin := public.is_auto_topup_admin_user\(v_user_id\)\s+and p_item_id <> 'BG-06';/);
  assert.match(sql,/p_item_id = 'BG-06' then\s+-- 프로필배경은 user_store_items 보유 기록만 있으면 인벤토리에서 표시 가능\s+null;/);
  ...
  assert.match(migration,/if position\('BG-06' in v_sql\) = 0 then/);
  assert.match(migration,/BG06_ITEM_ID_CONFLICT/);
  assert.match(migration,/BG06_EQUIP_OWNERSHIP_TRIGGER_MISMATCH/);
  assert.match(migration,/BG06_PURCHASE_SECURITY_MISMATCH/);
});
