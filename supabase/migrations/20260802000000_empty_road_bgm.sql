...
do $validate_empty_road_dependencies$
begin
  if to_regclass('public.profiles') is null
    ...
    raise exception 'EMPTY_ROAD_PURCHASE_TABLE_MISSING';
  end if;
  ...
  if to_regprocedure('public.purchase_store_item(text)') is null
    ...
    raise exception 'EMPTY_ROAD_PURCHASE_DEPENDENCY_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'EMPTY_ROAD_OWNERSHIP_UNIQUE_CONSTRAINT_MISSING';
  end if;

  if exists (
    select 1
    from public.user_store_items item
    where item.item_id = 'bgm-potato-02'
      and (
        item.item_name is distinct from '텅 빈 거리'
        or item.item_category is distinct from 'bgm'
        or item.purchase_price is distinct from 698
      )
  ) then
    raise exception 'EMPTY_ROAD_ITEM_ID_CONFLICT';
  end if;
end;
$validate_empty_road_dependencies$;

do $add_empty_road_to_purchase_store_item$
declare
  v_function_sql text;
...
begin
  ...
  if position($item_id$p_item_id = 'bgm-potato-02'$item_id$ in v_function_sql) > 0 then
    if position($price$p_item_id = 'bgm-potato-02' then
    v_price := 698;
    v_name := '텅 빈 거리';
    v_category := 'bgm';$price$ in v_function_sql) = 0
       or position($inventory$elsif p_item_id = 'bgm-potato-02' then
    null;$inventory$ in v_function_sql) = 0
       or position($message$then '텅 빈 거리 구매가 완료됐어. 698피클이 차감됐고 내프로필 BGM 인벤토리에서 선택할 수 있어.'$message$ in v_function_sql) = 0
       or (
      ...
      raise exception 'EMPTY_ROAD_EXISTING_BRANCH_MISMATCH';
    end if;

    return;
  end if;

  if position('bgm-potato-02' in v_function_sql) > 0 then
    raise exception 'EMPTY_ROAD_PURCHASE_FUNCTION_PARTIAL_STATE';
  end if;

  if position(v_price_anchor in v_function_sql) = 0 then
    raise exception 'EMPTY_ROAD_PRICE_ANCHOR_NOT_FOUND';
  end if;

  if position(v_inventory_anchor in v_function_sql) = 0 then
    raise exception 'EMPTY_ROAD_INVENTORY_ANCHOR_NOT_FOUND';
  end if;

  if position(v_message_anchor in v_function_sql) = 0 then
    raise exception 'EMPTY_ROAD_MESSAGE_ANCHOR_NOT_FOUND';
  end if;
  ...
  if v_anchor_count <> 2 then
    raise exception 'EMPTY_ROAD_STRICT_BALANCE_ANCHOR_COUNT_INVALID: %', v_anchor_count;
  end if;
...
$add_empty_road_to_purchase_store_item$;
...
do $verify_empty_road_purchase_store_item$
declare
  v_function_sql text;
  v_strict_balance_pattern text := $strict_balance_pattern$'bgm-tetocarrto-02',
        'bgm-potato-02',
...
begin
  ...
  if position($price$p_item_id = 'bgm-potato-02' then
    ...
    raise exception 'EMPTY_ROAD_PURCHASE_FUNCTION_VERIFY_FAILED';
  end if;
  ...
  if has_function_privilege('anon', 'public.purchase_store_item(text)', 'execute')
    ...
    raise exception 'EMPTY_ROAD_PURCHASE_FUNCTION_PERMISSION_VERIFY_FAILED';
  end if;
end;
$verify_empty_road_purchase_store_item$;

commit;

select json_build_object(
  'item_id', 'bgm-potato-02',
  'server_price', 698,
  'item_name', '텅 빈 거리',
  'item_category', 'bgm',
  'purchase_function_updated', position(
    $verify$p_item_id = 'bgm-potato-02'$verify$
    in pg_get_functiondef('public.purchase_store_item(text)'::regprocedure)
  ) > 0,
  'inventory_branch_present', position(
    $verify$elsif p_item_id = 'bgm-potato-02' then
    null;$verify$
    in pg_get_functiondef('public.purchase_store_item(text)'::regprocedure)
  ) > 0,
  'success_message_present', position(
    $verify$텅 빈 거리 구매가 완료됐어. 698피클이 차감됐고 내프로필 BGM 인벤토리에서 선택할 수 있어.$verify$
    in pg_get_functiondef('public.purchase_store_item(text)'::regprocedure)
  ) > 0,
  'strict_balance_list_occurrences', (
    length(pg_get_functiondef('public.purchase_store_item(text)'::regprocedure))
      - length(replace(
        ...
        $verify$'bgm-tetocarrto-02',
        'bgm-potato-02',
        'BF-02'$verify$,
        ''
      ))
  ) / length($verify$'bgm-tetocarrto-02',
        'bgm-potato-02',
        'BF-02'$verify$),
