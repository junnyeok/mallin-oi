...
-- - 거미줄 효과는 테스트 잔액 우회와 관리자 자동충전을 허용하지 않는다.
...
begin
  ...
  if v_strict_count = 0 then
    ...
    from regexp_matches(
      v_function_sql,
      $strict_anchor$'cha-effects-fire-01',([[:space:]]*)'bgm-tetocarrto-02'$strict_anchor$,
      'g'
    );

    if v_strict_count <> 2 then
      raise exception 'SPIDER_WEB_CHARACTER_EFFECT_STRICT_BALANCE_ANCHOR_MISMATCH';
    end if;

    v_function_sql := regexp_replace(
      v_function_sql,
      $strict_anchor$'cha-effects-fire-01',([[:space:]]*)'bgm-tetocarrto-02'$strict_anchor$,
      $strict_replacement$'cha-effects-fire-01',\1'cha-effects-web-01',\1'bgm-tetocarrto-02'$strict_replacement$,
      'g'
    );
  elsif v_strict_count <> 2 then
