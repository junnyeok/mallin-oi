...
export function configureSecondaryAudioSession(windowRef = window) {
  const audioSession = windowRef?.navigator?.audioSession;
...
export async function shouldPlaySecondaryAudio(windowRef = window) {
  const plugin = getNativePlugin(windowRef);
...
export async function shouldPlayCompletionSound(windowRef = window) {
  return shouldPlaySecondaryAudio(windowRef);
}
